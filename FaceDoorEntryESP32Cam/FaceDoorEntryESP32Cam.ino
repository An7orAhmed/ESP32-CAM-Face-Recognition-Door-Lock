...
#define relay 12 // pin 12 can also be used
#define redLed 13
#define greenLed 15
...
void open_door(WebsocketsClient &client) {
  if (digitalRead(relay) == LOW) {
    digitalWrite(relay, HIGH); //close (energise) relay so door unlocks
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (millis() - interval > door_opened_millis) { // current time - face recognised time > 5 secs
      if (digitalRead(relay)) Serial.println("Door locked");
      digitalWrite(relay, LOW); 
      digitalWrite(redLed, LOW);
      digitalWrite(greenLed, LOW);
    }
